...
1. Create a `.Renviron` with the eviction database credentials: `PG_DBNAME`, `PG_HOST`, `PG_PORT`, `PG_USER`, `PG_PASSWORD`.
2. Render with `quarto render`. Missing R packages are installed on first run.

## Outputs

- `docs/` — the rendered site
- `NYS_Evictions_2020_2025.xlsx` — compiled filings and executions by district
