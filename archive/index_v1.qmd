...
```{r}
eviction_count_df <- dbGetQuery(con, "
  SELECT
...
```{r}
eviction_count_annual_df <- dbGetQuery(con, "
  SELECT
...
```{r}
eviction_count_courts_df <- dbGetQuery(con, "
  SELECT
    court,
    COUNT(DISTINCT indexnumberid)::integer
  FROM oca_index
...
```{r}
eviction_count_courts_25_df <- dbGetQuery(con, "
SELECT
  court,
  COUNT(DISTINCT indexnumberid)::integer
FROM oca_index
